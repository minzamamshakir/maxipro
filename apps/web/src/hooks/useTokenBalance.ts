...
// AINZICS TODO
import { coreRpcProvider, bscRpcProvider } from '../utils/providers'
import { useTokenContract } from './useContract'
import { useSWRContract } from './useSWRContract'

const useTokenBalance = (tokenAddress: string, forceCore?: boolean) => {
  ...
  const key = useMemo(
    () =>
      account
        ? {
            contract: forceCore ? contract.connect(coreRpcProvider) : contract,
            methodName: 'balanceOf',
            params: [account],
          }
        : null,
    [account, contract, forceCore],
  )
...
export const useGetCakeBalance = () => {
  ...
  // AINZICS TODO
  const { balance, fetchStatus } = useTokenBalance(ICE[chainId]?.address || ICE[ChainId.CORE]?.address, true)
